...
name will then be used in the "CREATE FOREIGN TABLE ..." SQL commands and to
...
=item PGUSER and PGPASSWORD

These two directives are used to set the login user and password.

If you do not supply a credential with PGPASSWORD and you have installed the
Term::ReadKey Perl module, Ora2Pg will ask for the password interactively. If
PGUSER is not set it will be asked interactively too.
...
values set with PG_DSN, PGUSER and PGPASSWORD. If you want to fully override
...
able to use this feature, the PG_DSN, PGUSER and PGPASSWORD must be set. Then:
...
To run data validation you must have the oracle_fdw extension installed with
FDW_SERVER and PG_DSN set. See these configuration directives for more
explanation.

By default Ora2Pg will extract 10000 rows from both side, you can change this
value using directive DATA_VALIDATION_ROWS. When it is set to zero all rows
of the tables will be compared.
...
file data_validation.log written in the current directory. The number of
error before stopping the diff between rows can be controlled using the
