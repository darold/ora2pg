...
Usage: ora2pg [-dhpqv --estimate_cost --dump_as_html] [--option value]
    ...
    -C | --cdc_file file: File used to store/read SCN per table during export.
                        ...
                        is the file written by the --cdc_init option.
   ...
   --cdc_init        : use current SCN per table to export data and register
...
Oracle function to use to extract the type of geometry from a spatial column.
...
that FDW_SERVER and PG_DSN configuration directives are set.
...
changes after the first import. But you can use the --cdc_init option to
export data with registration of the SCN at the time of data export.
The SCN is written to a file named LAST_EXPORT_SCN.txt by default,
it can be changed using -C | --cdc_file option.

The SCN registered during COPY or INSERT export can be used with a CDC tool.
