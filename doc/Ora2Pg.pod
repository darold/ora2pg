...
Here are all command line parameters available when using ora2pg:
  ...
  Usage: ora2pg [-dhpqv --estimate_cost --dump_as_html] [--option value]
    ...
    -m | --mysql      : inform Ora2Pg to export a MySQL database instead of an
                        Oracle schema. Must be used when there is no database
			connection, otherwise it is autodetected from datasource.
...
=head2 Global Oracle and MySQL migraton assessment
