...
Here is a sample of report:
	...
	DATABASE LINK 3 0 Database links will be exported as SQL/MED PostgreSQL's Foreign Data Wrapper (FDW) extentions
				using oracle_fdw.
	...
	SYNONYM   3 0 SYNONYMs will be exported as views. SYNONYMs do not exists with PostgreSQL but a common workaround
				is to use views or set the PostgreSQL search_path in your session to access object outside
				the current schema.
...
to file like this: binmode OUTFH, ":utf8"; If you have change the NLS_LANG
...
Setting VIEW_AS_TABLE to product_prices and using export type TABLE, here is
the create table statment generated:

	CREATE TABLE product_prices (category_id, product_count, low_price, high_price) AS
	SELECT  category_id,
...
If you create the view as table before data has been loaded, they will be loaded
following the COPY export type and the view declaration.
...
The generated report is same as above but with a new 'Estimated cost' column as follow:
	...
	--------------------------------------
	DATABASE LINK  3 0 9 Database links will be exported as SQL/MED PostgreSQL's Foreign Data Wrapper (FDW) extentions
					using oracle_fdw.
...
Toad is also able to rewrite the native Oracle DECODE() syntax into ANSI standard SQL CASE statement.
