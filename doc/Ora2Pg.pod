...
PACKAGE, PARTITION, TYPE, INSERT or COPY, FDW.
...
The SHOW_COLUMN allow an other ora2pg command line option: '--xtable relname'
...
Particular case: if the object name is a view and the export type is TABLE,
the view will be exported as a create table statement. If export type is COPY
or INSERT, the corresponding data will be exported.
...
This directive is used to disables triggers on all tables in COPY or INSERT export
...
have to specify your views in the ALLOW configuration option.

Then if Ora2Pg does not find the name in Oracle table names it automaticaly
deduces that it must search for it in the view names, and if it finds the view
it will extract its schema (if TYPE=TABLE) into a PG create table form, then it
will extract the data (if TYPE=COPY or INSERT) following the view schema. 
