...
Use this directive to redefined the number of person-days limit where the
...
INSERT export type. If you want to give you custom export order, just give
a filename as value that contains the ordered list of table to export. Must
...
the number of hash partition to create. Default is none to not export the
...
By default the pgtt extension is loaded using the superuser privilege. Enabled it
...
To estimate the migration cost in person-days, Ora2Pg allow you to use a configuration
