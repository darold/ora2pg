...
but there's still manual workxs to do. The Oracle specific PL/SQL code generated
...
The Oracle Instant Client or a full Oracle instaaltion must be installed on
...
ORACLE_HOME will be the same and muist be set to the directory where you have
...
=item QUALIFY_FUNCTION

This directive is used to control the second pass done on output files to
requalify all function call to prefix them with the package/schema name.
Function calls fctname() will be translated to package.fctname(). Enabled
by default, disable it if you are sure that all function calls are well
qualified with their package/schema name. Disable it can speed up export
if you have lot of objects.
