...
Enable this directive if you want to load full content of BLOB and CLOB and
not use LOB locators. If enable you must take care of the value of LONGREADLEN.
This directive is now disabled by default as using a lob locator has better
performances when chunk size can be controled, see LOB_CHUNK_SIZE.
...
functions this is not represent the real migration time.
