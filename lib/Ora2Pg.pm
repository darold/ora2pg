...
sub _init
{
	...
	# Exclude table generated by partition logging, materialized view logs, statistis on spatial index,
	# spatial index tables, sequence index tables, interMedia Text index tables and Unified Audit tables.
	push(@{$self->{excluded}{TABLE}}, @EXCLUDED_TABLES);
	...
	foreach my $k (keys %options) {
		if ($options{allow} && (lc($k) eq 'allow')) {
			...
			my @allow_detail = split(/\s*;\s*/, $options{allow});
			foreach my $a (@allow_detail) {
				if ($a =~ /^([^\[]+)\[(.*)\]$/) {
			...
			$self->{excluded} = ();
			# Syntax: TABLE[regex1 regex2 ...];VIEW[regex1 regex2 ...];glob_regex1 glob_regex2 ...
			my @exclude_detail = split(/\s*;\s*/, $options{exclude});
			foreach my $a (@exclude_detail) {
				if ($a =~ /^([^\[]+)\[(.*)\]$/) {
			...
			push(@{$self->{excluded}{ALL}}, @EXCLUDED_TABLES);
...
sub _column_comments
{
	...
	$condition .= $self->limit_to_objects('TABLE') if (!$table);
...
sub _unique_key
{
	...
	$condition .= "AND TABLE_NAME='$table' " if ($table);
	$condition .= "AND OWNER='$owner' " if ($owner);
	$condition .= $self->limit_to_objects('TABLE|UKEY') if (!$table);
...
sub _check_constraint
{
	...
	$condition .= "AND TABLE_NAME='$table' " if ($table);
	$condition .= "AND OWNER='$owner' " if ($owner);
	$condition .= $self->limit_to_objects('TABLE|CKEY') if (!$table);
...
sub _foreign_key
{
	...
	$condition .= "AND TABLE_NAME='$table' " if ($table);
	$condition .= "AND OWNER='$owner' " if ($owner);
	$condition .= $self->limit_to_objects('TABLE|FKEY') if (!$table);
...
sub _get_indexes
{
	...
	$condition .= $self->limit_to_objects('TABLE|INDEX', "$self->{prefix}_IND_COLUMNS.TABLE_NAME") if (!$table);
...
sub _get_triggers
{
	...
	$str .= " " . $self->limit_to_objects('TABLE|TRIGGER');
...
sub _table_info
{
	...
	$owner .= $self->limit_to_objects('TABLE');
...
sub _get_synonyms
{
	...
	$str .= $self->limit_to_objects('SYNONYM');
...
sub read_config
{
	...
	while (my $l = <$fh>) {
		...
		$var = uc($var);
			...
			# Syntax: TABLE[regex1 regex2 ...];VIEW[regex1 regex2 ...];glob_regex1 glob_regex2 ...
			my @detail = split(/\s*;\s*/, $AConfig{$var});
			foreach my $a (@detail) {
				if ($a =~ /^([^\[]+)\[(.*)\]$/) {
...
sub limit_to_objects
{
	...
	foreach my $obj (split(/\|/, $obj_type)) {
		...
		if ($#{$self->{limited}{$obj}} >= 0) {
			$str .= ' OR (';
			if ($self->{db_version} =~ /Release [89]/) {
				$str = " $column IN ('" .  join("','", @{$self->{limited}{$obj}}) . "') ";
			} else {
				for (my $j = 0; $j <= $#{$self->{limited}{$obj}}; $j++) {
					$str .= " REGEXP_LIKE($column,'\^$self->{limited}{$obj}->[$j]\$')" ;
					if ($j < $#{$self->{limited}{$obj}}){
						$str .= " OR ";
					}
				}
			}
			...
			$str .= ' AND (';
			if ($self->{db_version} =~ /Release [89]/) {
				$str = "$column NOT IN ('" .  join("','", @{$self->{excluded}{$obj}}) . "') ";
			} else {
				for (my $j = 0; $j <= $#{$self->{excluded}{$obj}}; $j++) {
					$str .= " NOT REGEXP_LIKE($column,'\^$self->{excluded}{$obj}->[$j]\$')" ;
					if ($j < $#{$self->{excluded}{$obj}}){
						$str .= " AND ";
					}
				}
			}
