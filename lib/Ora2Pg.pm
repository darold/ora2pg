...
our @EXCLUDED_TABLES = ('USLOG\$_.*', 'MLOG\$_.*', 'RUPD\$_.*', 'MDXT_.*', 'MDRT_.*', 'MDRS_.*', 'DR\$.*', 'CLI_SWP\$.*', 'LOGMNR\$.*', 'REPCAT\$.*', 'JAVA\$.*','AQ\$.*');
our @EXCLUDED_TABLES_8I = ('USLOG$_%', 'MLOG$_%', 'RUPD$_%', 'MDXT_%', 'MDRT_%', 'MDRS_%', 'DR$%', 'CLI_SWP$%', 'LOGMNR$%', 'REPCAT$%', 'JAVA$%', 'AQ$%');
...
sub limit_to_objects
{
	...
	for (my $i = 0; $i <= $#arr_type; $i++) {
		...
		if ($arr_type[$i] =~ /TABLE|SEQUENCE|VIEW/) {
			if ($self->{db_version} =~ /Release [89]/) {
				$str .= ' AND (';
